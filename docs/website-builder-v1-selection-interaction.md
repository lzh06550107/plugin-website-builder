...
复制样式 / 粘贴样式需要独立的编辑器样式剪贴板状态，不在本小步实现。
...
9. 当前选中 Text 时直接右键 Button 删除，只删除 Button，不能误删 Text。
10. 右键 Page 时复制/上移/下移/删除均不可执行。
11. Preview 中不出现面包屑、Hover Outline、Context Menu、Drop Indicator 或其他 Editor Chrome。
