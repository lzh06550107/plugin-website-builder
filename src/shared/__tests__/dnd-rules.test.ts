...
test('validates palette and existing-node drops against strict registry hierarchy', () => {
  ...
  assert.equal(
    validateDropSource(root, componentRegistry, { kind: 'palette', componentType: 'wb.text' }, {
      parentId: 'grid-1', index: 1, position: 'inside',
    }).valid,
    true,
  );
  assert.equal(
  ...
  assert.equal(
    validateDropSource(root, componentRegistry, { kind: 'palette', componentType: 'wb.grid' }, {
  ...
  assert.equal(
    validateDropSource(root, componentRegistry, { kind: 'node', nodeId: 'container-1' }, {
      parentId: 'grid-1', index: 0, position: 'inside',
    }).valid,
    false,
  );
});
